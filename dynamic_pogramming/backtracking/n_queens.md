...
1. Écrire une fonction `print_board(board)` qui affiche l'échiquier avec des `Q` et des `.`.
2. Écrire une fonction `count_n_queens(n)` qui compte uniquement le nombre de solutions **sans les stocker en mémoire**.
3. Optimiser `is_safe` en utilisant des `set()` pour suivre en O(1) les colonnes, diagonales `/` et diagonales `\` déjà occupées, au lieu de parcourir `board` à chaque appel.
...
print("Test 5...", len(solve_n_queens(5)))
assert len(solve_n_queens(5)) == 10, "Échec Test 5 : n=5 doit avoir 10 solutions"
